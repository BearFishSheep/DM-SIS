...
def four_to_one(s):
    ...
    for i in range(len(s)):
        ...
        for j in range(0, len(s[i]), 4):
            ...
            # temp1 = []
            for num in range(4):
                total = total * 256 + s[i][j + num]
                # temp1.append(s[i][j + num])
            # total = temp1[0] * 256 ** 3 + temp1[1] * 256 ** 2 + temp1[2] * 256 + temp1[3] * 256
            temp.append(int(str(bin(total)).replace('0b', '').zfill(32), 2))
        s_temp.append(temp)
    return s_temp


def one_to_four(s_recover):
    s_recover_temp = []
    for i in range(len(s_recover)):
        temp = []
        for j in range(len(s_recover[0])):
            x = bin(s_recover[i][j]).replace('0b', '').zfill(32)
            for num in range(4):
                temp.append(int(x[num * 8: num * 8 + 8], 2))
...
def main():
    ...
    s = [np.array(s1).flatten().astype(np.int64), np.array(s2).flatten().astype(np.int64),
         np.array(s3).flatten().astype(np.int64), np.array(s4).flatten().astype(np.int64)]  # 秘密

    s = four_to_one(s)
    # print('s: ', s)

    # GAS = [[1, 2], [3, 4], [2, 3, 4], [1, 2, 3]]  # 访问结构1，m = n
    # GAS = [[1, 2], [2, 3], [1, 3], [1, 2, 3]]
    GAS = [[1, 4, 5], [2, 4], [1, 3], [3, 5]]
    ...
    rands = [GF(np.random.randint(1, 65536 ** 2, dtype=np.int64)) for _ in range(max(max(m, n), len(M)))]  # 增加随机数干扰项，提高安全性
    ...
    if m < n or (m == n and len(M) > n):
        ...
        for _ in range(num):
            ...
            s.append([np.random.randint(65536 ** 2, dtype=np.int64) for _ in range(len(s[0]))])
    # print('s:', s)
    ...
    s_recover_ultimate = one_to_four(s_recover_ultimate)
    ...
    #         .save("./sis_32/mLTn(31-34)/reconstruct_{}.jpg".format(i + 1))
