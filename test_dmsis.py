...
def main():
    ...
    #      np.array(s3).flatten(), np.array(s4).flatten()]  # 秘密
    s = [np.array(s1).flatten(), np.array(s2).flatten(),
         np.array(s3).flatten(), np.array(s4).flatten(),
         np.array(s5).flatten(), np.array(s6).flatten()]  # 秘密

    # GAS = [[1, 2], [1, 3], [2, 3], [3, 4], [1, 2, 3], [2, 3, 4]]
    # GAS = [[1, 2], [1, 3], [1, 4], [2, 3, 4]]
    # GAS = [[1, 2], [3, 4], [1, 2, 3], [1, 2, 3, 4]]
    # GAS = [[1, 2, 3], [1, 2, 4], [1, 3, 4], [2, 3, 4]]    (3, 4)
    # GAS = [[1, 2], [1, 3], [1, 4], [2, 3], [2, 4], [3, 4]]      # (2, 4)

    # GAS = [[1, 2], [1, 3], [1, 2, 3]]  # 访问结构1，m = n
    # GAS = [[1, 4], [2, 3, 5], [1, 2, 3, 4], [1, 3], [2, 4, 5]]  # 访问结构1，m = n
    # GAS = [[1, 2], [3, 4], [1, 2, 3, 4], [1, 3]]  # 访问结构1，m = n
    # GAS = [[1, 2], [3, 4], [2, 3, 4], [1, 2, 3]]  # 访问结构1，m = n
    # GAS = [[2, 4], [1, 2, 3, 4], [2, 3, 4], [2, 3]]  # 访问结构1，m = n
    # GAS = [[1, 2, 4], [1, 2, 3], [2, 3, 4], [1, 3, 4]]  # 访问结构1，m = n
    # GAS = [[1, 2], [3, 4], [2, 3, 4], [1, 2, 3], [3, 5]]  # 访问结构1，m = n
    # GAS = [[1, 4], [3, 4], [2, 3, 4], [1, 2, 3], [2, 4]]  # 访问结构1，m = n
    # GAS = [[1, 2], [2, 3], [1, 3], [1, 2, 3]]  # 访问结构1，m = n
    # GAS = [[1, 2], [1, 3], [2, 3], [2, 4], [3, 4], [1, 2, 3]]   # 访问结构2，m > n
    GAS = [[1, 3, 4], [1, 2], [2, 3, 4], [1, 2, 4], [1, 3], [1, 2, 3]]   # 访问结构2，m > n
    # GAS = [[1, 4], [1, 3, 4], [2, 3, 4], [2, 4], [3, 4], [1, 2, 3]]   # 访问结构2，m > n
    # GAS = [[1, 2], [2, 4], [1, 2, 3, 4], [3, 4], [2, 3, 4], [1, 2, 3]]   # 访问结构2，m > n
    # GAS = [[1, 3], [2, 3], [1, 2, 4], [1, 2, 3], [2, 3, 4]]  # 访问结构2，m > n
    # GAS = [[1, 3], [2, 3, 4], [1, 2, 3, 4], [1, 3, 4], [3, 4]]  # 访问结构2，m > n
    # GAS = [[1, 4], [2, 3], [1, 3, 4]]  # 访问结构3，m < n
    # GAS = [[1, 2], [1, 3], [1, 2, 3]]  # 访问结构3，m < n
    # GAS = [[2, 3, 4], [1, 3, 4], [1, 2, 4], [1, 2, 3]]  # 访问结构3，m < n
    # GAS = [[2, 3, 4], [3, 4, 5], [1, 2, 4], [2, 4, 5], [1, 2, 3]]  # 访问结构3，m < n
    # GAS = [(2, 3, 4), (3, 4, 5), (1, 2, 4), (2, 4, 5), (1, 2, 3)]  # 访问结构3，m < n
    # GAS = [[1, 2], [3, 5], [2, 4]]  # 访问结构3，m < n
    # GAS = [[1, 5], [2, 3], [1, 3, 4], [2, 4]]  # 访问结构3，m < n
    # GAS = [[1, 2], [2, 3], [1, 3, 4], [2, 4]]  # 访问结构3，m < n
    # GAS = [[1, 2], [2, 3], [1, 3], [1, 2, 3]]  # 访问结构3，m < n，当m=4，n=5时这个有问题
    # GAS = [[1, 2, 4], [2, 3], [1, 3], [1, 2, 3]]  # 访问结构3，m < n
    # GAS = [[1, 2], [3, 6], [4, 5], [2, 3, 4]]  # 访问结构3，m < n
    # GAS = [[1, 2], [3, 5], [4, 5], [2, 3, 4]]  # 访问结构3，m < n
    # GAS = [[1, 2], [3, 4], [1, 2, 4], [1, 3]]  # 访问结构3，m < n
    # GAS = [[1, 3], [3, 4], [1, 2, 4]]  # 访问结构3，m < n
    # GAS = [[1, 2, 4], [1, 3], [2, 3], [1, 2, 3]]  # 访问结构3，m < n
    GAS_matrix = modify.get_specific_matrix(GAS, len(GAS), n)
    print('GAS_matrix:', GAS_matrix)

    M = get_M(max(m, n))
    # M = Demo.get_M(4, [2, 7, 9, 11])
    print('M:', M)
    ...
    #         .save("./Image Random Safety modify/k plus 1/share_{}.bmp".format(i + 1))
    ...
    #         .save("./Image Random Safety modify/k plus 1/reconstruct_{}.bmp".format(i + 1))
